...
public class Article {
     ...
     *
     * @return articleType - 文章类型，1：图文（1张封面），2：纯文字
     */
     ...
     *
     * @return articleCover - 文章封面图，article_type=1 的时候展示
     */
     ...
     *
     * @return isAppoint - 是否是预约定时发布的文章，1：预约（定时）发布，0：即时发布    在预约时间到点的时候，把1改为0，则发布
     */
     ...
     *
     * @return articleStatus - 文章状态，1：审核中（用户已提交），2：机审结束，等待人工审核，3：审核通过（已发布），4：审核未通过；5：文章撤回（已发布的情况下才能撤回和删除）
     */
     ...
     * 获取文章发布时间（也是预约发布的时间）
     *
     * @return publishTime - 文章发布时间（也是预约发布的时间）
     */
     ...
     *
     * @return readCounts - 用户累计点击阅读数（喜欢数）（点赞） - 放redis
     */
     ...
     *
     * @return commentCounts - 文章评论总数。评论防刷，距离上次评论需要间隔时间控制几秒
     */
     ...
     *
     * @return isDelete - 逻辑删除状态，非物理删除，1：删除，0：未删除
     */
